...
async def execute_account_row_click(page, step_name: str, test_data: str | None = None):
    ...
    # =================================================
    # PRIORITY 1: TEST DATA (BEST PRACTICE)
    # =================================================

    # Case A: masked value in test data (XXXXXXXX5034)
    if test_data and "xxxx" in data_lower:
        last4 = data_lower[-4:]
    ...
    # Case B: full / partial account number in test data
    if link_to_click is None and test_data:
        digits = "".join(c for c in test_data if c.isdigit())
    ...
    if link_to_click is None:
        raise RuntimeError(
            "Unable to determine which account to click (step + test data empty)"
        )
